...
module CalendarDateSelect::FormHelpers
  ...
  def calendar_date_select_tag( name, value = nil, options = {})
    ...
    options[:size] ||= 10
    javascript_options[:time] ||= false
    javascript_options[:buttons] ||= false
    options_for_text_field = options.dup
    options_for_text_field.delete :image
    tag = javascript_options[:hidden] || javascript_options[:embedded] ?
      hidden_field_tag(name, value, options_for_text_field) :
      text_field_tag(name, value, options_for_text_field)
  ...
  def calendar_date_select(object, method, options={})
    ...
    if !options.include?(:time) && obj.class.respond_to?("columns_hash")
      ...
      options[:time] = false if column_type == :datetime
    end
