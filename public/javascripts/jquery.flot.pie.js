 ...
 *
 * original PIE modifications by Sergey Nosenko http://javascript.cooldev.com
 * Additional PIE modifications by Brian Medendorp, May 2009
 */
    ...
    function Plot(target_, data_, options_) {
        ...
        options = {
            ...
            pie: {
                ...
                centerOffsetTop:0,
                centerOffsetLeft:'auto',
                showLabel: false,
                labelFormatter: null,
                labelOffsetFactor: 3/4, //as part of radius
                labelOffset: 0,
                labelBackgroundOpacity: 0.0,
                labelBackgroundColor: null, // null means auto-detect (pie slice color)
                labelHidePercent: 0,
                radiusLimitFactor: 1,
                radiusLimit: 0,
                sliceCombinePercent: 0,
                sliceCombineColor: null,
                sliceCombineLabel: 'Other'
            },
            crosshair: {
                mode: null, // one of null, "x", "y" or "xy",
                color: "#aa0000"
        ...
        //PIE: handle redrawing if labels outside canvas (and hard pixels values not set)
        if (options.pie.show && !(options.pie.labelOffset || options.pie.radiusLimit)) {
            var attempts = 1;
            while (pieRedraw && attempts < pieRedrawAttempts) {
                pieRedraw = false;
                pieChartRadius *= 0.95;
                constructCanvas();
                setupGrid();
        ...
        function setData(d) {
            ...
            //PIE: combine slices if necessary
            if (options.pie.show) {
                pieTotal = 0;
                ...
                var temp = [];
                for (var i=0;i<series.length;i++) {
                    if ((series[i].data/pieTotal)*100<=options.pie.sliceCombinePercent)
                        pieOther += series[i].data;
                    else
                        temp.push(series[i]); // add back this slice
                }
                series = temp;
                if (pieOther>0)
                    series.push( {
                        label: options.pie.sliceCombineLabel,
                        data: pieOther,
                        color: options.pie.sliceCombineColor
                    } );
            }
        ...
        function setupGrid() {
            ...
            if (options.pie.show) {
                pieChartRadius = pieChartRadius || Math.min(canvasWidth,canvasHeight)/2;
                pieChartDiameter = pieChartRadius * 2;
                insertLegend();
                pieCenterLeft = canvasWidth/2;
                pieCenterTop = canvasHeight/2;
				
                if (options.pie.centerOffsetLeft=='auto')
                    if (options.legend.position.match('w'))
                        pieCenterLeft += legendWidth;
                    else
                        pieCenterLeft -= legendWidth;
                else
                    pieCenterLeft += options.pie.centerOffsetLeft;
					
                if (pieCenterLeft<pieChartRadius)
                    pieCenterLeft = pieChartRadius;
                else if (pieCenterLeft>canvasWidth-pieChartRadius)
                    pieCenterLeft = canvasWidth-pieChartRadius;
        ...
        //PIE: draw series pie
        function drawSeriesPie(series){
            //if (series.percent<=options.pie.sliceCombinePercent)
            //	return;
            var thisvalue = series.data / pieTotal;
            ...
            var actualRadius = series.pie.radiusLimit?series.pie.radiusLimit:pieChartRadius*series.pie.radiusLimitFactor;
            ...
            ctx.closePath();
            ctx.fillStyle = series.color; // Color
            ctx.fill();
			
            // Draw the same thing again to draw stroke properly in IE
            if (series.pie.pieStrokeLineWidth > 0) {
                ctx.strokeStyle = series.pie.pieStrokeColor;
                ctx.lineWidth = series.pie.pieStrokeLineWidth;
                ctx.beginPath();
                ctx.moveTo(pieCenterLeft,pieCenterTop); // Center of the pie
                ctx.arc(  // draw next arc
                    pieCenterLeft,
            ...
            if (options.pie.showLabel && series.percent>options.pie.labelHidePercent) {
                var lf = options.legend.labelFormatter, text, plf = options.pie.labelFormatter;
                if (lf)
                    text = lf(series.label, series);
                else
                    text = series.label;
                if (plf)
                    text = plf(text, series);
				
                var halfAngle = (arcEndAngle + arcStartAngle)/2;
                var lo = series.pie.labelOffset?series.pie.labelOffset:pieChartRadius*series.pie.labelOffsetFactor;
                var xh = pieCenterLeft + Math.round(Math.cos(halfAngle) * lo);
                var yh = pieCenterTop + Math.round(Math.sin(halfAngle) * lo);
                var html = '<span class="pieLabel" id="pieLabel'+pieArcIndex+'" style="position:absolute;top:' + (plotOffset.top + yh) + 'px;left:' + (plotOffset.left + xh) + 'px;">' + text + "</span>";
                target.append(html);
                var label = target.children('#pieLabel'+pieArcIndex);
                var labelTop = (plotOffset.top + yh - label.height()/2);
                var labelLeft = (plotOffset.left + xh - label.width()/2);
                ...
                if (options.pie.labelBackgroundOpacity != 0.0) {
                    // put in the transparent background
                    // separately to avoid blended labels and
                    // label boxes
                    var c = options.pie.labelBackgroundColor;
                    if (c == null) {
                        c = series.color;
                    }
                    var pos = 'top:'+labelTop+'px;left:'+labelLeft+'px;';
                    $('<div style="position:absolute;width:' + label.width() + 'px;height:' + label.height() + 'px;' + pos +'background-color:' + c + ';"> </div>').insertBefore(label).css('opacity', options.pie.labelBackgroundOpacity);
                }
            }
        ...
        function getSelectionForEvent() {
            ...
            if (axes.xaxis.used)
                r.xaxis = {
                    from: axes.xaxis.c2p(x1),
                    to: axes.xaxis.c2p(x2)
                };
            if (axes.x2axis.used)
                r.x2axis = {
                    from: axes.x2axis.c2p(x1),
                    to: axes.x2axis.c2p(x2)
                };
            if (axes.yaxis.used)
                r.yaxis = {
                    from: axes.yaxis.c2p(y1),
                    to: axes.yaxis.c2p(y2)
                };
            if (axes.y2axis.used)
                r.y2axis = {
                    from: axes.y2axis.c2p(y1),
                    to: axes.y2axis.c2p(y2)
                };
            return r;
        }
